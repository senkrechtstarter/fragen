...
export class Handler {
  ...
  constructor(private chatID: string, public videoID: string, public mode = ['@mo', '@senkrecht','@senkrechtstarter']) {
  ...
  async handler() {
    ...
    while(true) {
      ...
      data.items.sort((a,b) => a.snippet.publishedAt > b.snippet.publishedAt ? -1 : 1)
      if(data.pollingIntervalMillis < MIN_TIMEOUT) data.pollingIntervalMillis = MIN_TIMEOUT
      
      for (let i = 0; i < data.items.length; i++) {
        const el = data.items[i];

        if (this.lastMessageTime !== null && el.snippet.publishedAt <= this.lastMessageTime) {
          setTimeout(this.handler, data.pollingIntervalMillis)
          this.lastMessageTime = last || data.items[0]?.snippet.publishedAt  || this.lastMessageTime
          page = null
          return
        }
  ...
  async handleMessage(msg: Message) {
    ...
    let author = `${msg.authorDetails.displayName}${msg.authorDetails.isChatSponsor ? ' | Member' : ''}`
    ...
    if(msg.snippet.type === 'memberMilestoneChatEvent') {
      ...
      author += `(${msg.snippet.memberMilestoneChatDetails!.memberLevelName} | ${msg.snippet.memberMilestoneChatDetails!.memberMonth})`
      isSuper = true
    }
